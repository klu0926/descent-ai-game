...
export function presentScavengerPotionReward(ctx) {
    ...
    card.innerHTML = `
        ...
        <div class="loot-name rarity-${item.rarity}" style="font-size: 1.05rem;">${item.displayName}</div>
        <div class="loot-desc">${item.storyDesc || item.desc}</div>
        <div class="loot-stats" style="font-size: 1.15rem;">${item.functionDesc || "Heal 25% Max HP"}</div>
    `;
    card.onclick = () => {
...
export function presentLootSelection(ctx) {
    ...
    options.forEach(item => {
        ...
        card.innerHTML = `
            <div class="loot-icon">${getLootIconMarkup(item)}</div>
            <div class="loot-name rarity-${item.rarity}" style="font-size: 1.05rem;">${item.displayName} <br><span style="font-size:0.75em; color:gray">Lv.${ctx.currentGameStats.currentLevel}</span></div>
...
export function selectLoot(ctx, item) {
    if (item.rewardType === "healing") {
        const healAmount = Math.floor(ctx.playerInfo.maxHp * (item.healPercent || 0));
        ...
        const openSlot = ctx.getNextOpenGearSlot();
        if (openSlot) {
