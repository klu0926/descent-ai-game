...
export function applySkillTreeTurnEffectsFlow(ctx) {
    ...
    if (ctx.gameEventBus && ctx.GAME_EVENTS) {
        if (battleState.turnOwner === "player" && ctx.GAME_EVENTS.PLAYER_TURN_START) {
            ctx.gameEventBus.emit(ctx.GAME_EVENTS.PLAYER_TURN_START, {
                turnOwner: "player",
                playerTurnCount: battleState.playerTurnCount,
                enemyTurnCount: battleState.enemyTurnCount,
                turnCount: battleState.turnCount,
                currentTurnNumber: battleState.currentTurnNumber
            });
        }
        if (battleState.turnOwner === "enemy" && ctx.GAME_EVENTS.ENEMY_TURN_START) {
            ctx.gameEventBus.emit(ctx.GAME_EVENTS.ENEMY_TURN_START, {
                turnOwner: "enemy",
                playerTurnCount: battleState.playerTurnCount,
                enemyTurnCount: battleState.enemyTurnCount,
                turnCount: battleState.turnCount,
    ...
    if (typeof ctx.onTurnStarted === "function") {
        ctx.onTurnStarted({
...
export async function playerAttacksFlow(ctx) {
    const { currentGameStats, playerInfo } = ctx;
    if (ctx.gameEventBus && ctx.GAME_EVENTS && ctx.GAME_EVENTS.PLAYER_ATTACK) {
        ctx.gameEventBus.emit(ctx.GAME_EVENTS.PLAYER_ATTACK, { source: "basic_attack" });
    }
...
export async function enemyAttacksFlow(ctx) {
    const { currentGameStats, playerInfo } = ctx;
    if (currentGameStats.currentEnemy && currentGameStats.currentEnemy.canAttack === false) {
        ctx.floatText("enemy", "Cannot attack", "info");
        await new Promise(resolve => setTimeout(resolve, 1000));
        currentGameStats.isPlayerTurn = true;
        return;
    }
    if (ctx.gameEventBus && ctx.GAME_EVENTS && ctx.GAME_EVENTS.ENEMY_ATTACK) {
        ctx.gameEventBus.emit(ctx.GAME_EVENTS.ENEMY_ATTACK, { source: "enemy_attack" });
    }
...
export function handleWinFlow(ctx) {
    const { currentGameStats, playerInfo } = ctx;
    const defeatedEnemy = currentGameStats.currentEnemy;
    ctx.gameLoop.stop();
    ...
    const gainedEssence = Math.max(0, Math.floor(Number(defeatedEnemy && defeatedEnemy.essence) || 0));
    if (gainedEssence > 0) {
        playerInfo.essence = Math.max(0, Math.floor(Number(playerInfo.essence) || 0)) + gainedEssence;
        ctx.floatText("player", `+${gainedEssence} Essence`, "info");
    }
...
export function advanceToNextRoundFlow(ctx) {
    ...
    const nextRound = (Number(ctx.currentGameStats.currentScene) || 1) + 1;
    ctx.floatText("system", `Scene ${nextRound}`, "system");
    setTimeout(() => {
...
export function handleLossFlow(ctx) {
    ...
    document.getElementById("modal-desc").innerText = `Journey ended at Scene ${ctx.currentGameStats.currentScene}. Fights cleared: ${ctx.currentGameStats.currentRound}.`;
